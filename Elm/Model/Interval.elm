...
compare (Interval intA) (Interval intB) =
    let
        selectEnd = case (intA.end, intB.end) of
            (Nothing, Nothing) -> EQ
            (Just endA, Nothing) -> LT
            (Nothing, Just endB) -> GT
            (Just endA, Just endB) -> Date.compare endA endB
    in
        chainCompare selectEnd (Date.compare intA.start intB.start)
    
    --Debug.todo "Implement Model.Interval.compare"


getIntervalStart: Interval -> Date
getIntervalStart (Interval inter) = inter.start

getIntervalEnd: Interval -> Maybe Date
getIntervalEnd (Interval inter) = inter.end

view : Interval -> Html msg
view interval =
    div [class "interval"] [
        div[class "interval-start"][(getIntervalStart >> Date.view) interval],
        div[class "interval-end"][getIntervalEnd interval |> Maybe.map Date.view |> Maybe.withDefault (text"Present")],
        div [class "interval-length"][
            case length interval of
                Nothing -> text ""
                Just (a, b) -> text (String.fromInt a ++ " " ++ String.fromInt b)
            ]
    ]
