...
monthsBetween dA dB =
    let
      (Date dA1) = dA
      (Date dB1) = dB
      (monthA, yearA) = (dA1.month, dA1.year)
      (monthB, yearB) = (dB1.month, dB1.year)
    in
    case (monthA, monthB) of
        (Nothing, Nothing) -> Just ((abs(yearA - yearB) * 12))  
        (Just monthA1, Just monthB1) -> Just (abs ((yearB - yearA) * 12 + monthToInt monthB1 - monthToInt monthA1))
        (_, _) -> Nothing
...
compare (Date d1) (Date d2) =
    chainCompare (Basics.compare
                    (d1.month |> Maybe.map monthToInt |> Maybe.withDefault 13)
                    (d2.month |> Maybe.map monthToInt |> Maybe.withDefault 13))
...
view (Date d) =
    div[][
        case (d.month) of
            ...
            (Just monthD) -> text(monthToString monthD ++ "-" ++ String.fromInt d.year)
    ]
